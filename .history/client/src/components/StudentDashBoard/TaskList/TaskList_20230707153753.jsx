...
const TaskList = () => {
  ...
  const addTask = (task) => {
    setTasks((prevTasks) => {
      ...
      const taskObj = { id: taskId, content: task }; // Create task object
      if (updatedTasks[curr_date]) {
        // Check if the task already exists for the current date
        if (!updatedTasks[curr_date].some((t) => t.content === task)) {
          updatedTasks[curr_date].push(taskObj);
        }
      } else {
        updatedTasks[curr_date] = [taskObj];
      }
      return updatedTasks;
    });
  };
  const removeTask = (taskId) => {
    setTasks((prevTasks) => {
      const updatedTasks = { ...prevTasks };
      updatedTasks[curr_date] = updatedTasks[curr_date].filter(
        (task) => task.id !== taskId
      );
      return updatedTasks;
    });
