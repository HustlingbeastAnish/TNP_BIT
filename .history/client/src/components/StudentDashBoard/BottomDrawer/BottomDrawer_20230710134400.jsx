...
function BottomDrawer() {
  ...
  // Function to check the screen size and set the initial drawer state
  const checkScreenSize = () => {
    const isMediumScreen = window.innerWidth <= 768;
    setIsDrawerOpen(!isMediumScreen);
  };
  // Listen for screen size changes on mount and window resize
  useEffect(() => {
    checkScreenSize();
  ...
  if (!isDrawerOpen) {
    return null; // If the drawer is closed, return null to hide it
  }
  return (
    <div>
      <div
        id="drawer-swipe"
        className={`fixed z-40 w-full overflow-y-auto bg-white border-t border-gray-200 rounded-t-lg dark:border-gray-700 dark:bg-gray-800 transition-transform bottom-0 left-0 right-0 translate-y-full bottom-[${bottom_width}px] md:hideen`}
        tabIndex="-1"
