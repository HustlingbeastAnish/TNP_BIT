...
exports.addAcademics = async (req, res) => {
  try {
    const { name, email, roll, college, schooll } = req.body;
    if (!name || !email || !roll || !schooll.class10 || !schooll.class12) {
      return res
        .status(400)
        .json({ error: "Class 10 and Class 12 fields cannot be empty" });
    }

    let student = await Academics.findOne({ email });
    ...
    student.school = {
      class10: {
        board: schooll.class10.board,
        percentage: schooll.class10.percentage,
      },
      class12: {
        board: schooll.class12.board,
        percentage: schooll.class12.percentage,
      },
    };
