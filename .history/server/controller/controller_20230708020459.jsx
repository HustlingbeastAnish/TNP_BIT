...
exports.editUser = async (req, res) => {
  try {
    ...
    const rollExists = await StudentLogin.findOne({ email: email });
    ...
    const id = rollExists._id;
    console.log("id:", id);
    StudentLogin.findByIdAndUpdate(id, req.body, { useFindAndModify: false })
      .then((data) => {
        if (!data) {
          res.status(404).send({
            message: `Cannot update a user with id ${id}, user not found`,
          });
        } else {
